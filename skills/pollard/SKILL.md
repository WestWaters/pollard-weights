...
| **HYV4** (Tencent Hy4-preview / hy_v4 — MLA + DSA-indexer + hyper-connection MoE) | **IS a MoE → runs through THE MoE recipe** (`automap` auto-detects it and applies the same one). The MoE `attn_q/k/v` rules substring-match HY4's MLA (`attn_k`→`k_b`/`kv_a_mqa`, `attn_v`→`v_b`, `attn_q`→`q_a`/`q_b`); the recipe adds `attn_gate`/`hc_*_fn`/`indexer.proj`/`output_hc_fn`/dense-blk0 (no-op on Qwen3). Same policy, same tiers | ⛔ do NOT build a separate recipe from a buggy run · needs a COVERED imatrix (256 experts — the coverage lesson at scale) |
